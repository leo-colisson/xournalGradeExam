...
- Automatically deal with questions that are not filled in the appropriate order (just add an empty grade when you see a question that is not properly positioned).
- You can either grade in points or in percentage.
- Option to automatically fill the clipboard with the template of the current grade to fill to easily copy/paste it in a text area (it the future we aim to have even easier way to add grades by simply clicking on the page, but xournal++ currently lacks the appropriate API).
- Grades can be exported to CSV and YAML in a pre-defined order based on an existing list of student, e.g. copy/pasted from an existing reference Excel file. This way you don't need to manually alphabetically sort exams, just paste the whole block in your template!
- We allow easy export of the grade in a CSVs that you can import in Excel/Libre office. You can specify for each question the maximum score used for instance when grading in percentage.
...
- To use the features to quickly assign a student to its exam or add a previously typed comment, you need to install `rofi` on Linux, [`wlines.exe` on windows](https://github.com/JerwuQu/wlines) (not tested, please give me returns), and [`choose` on MacOS](https://github.com/chipsenkbeil/choose). To install `wlines.exe` on windows, you can copy the downloaded file to an arbitrary folder, and change the `PATH` environment variable as [described here](https://learn.microsoft.com/en-us/previous-versions/office/developer/sharepoint-2010/ee537574(v=office.14)) to add this folder so that our plugin knows where to find it. You can live without this dependency, but you'll need to manually type the name of each student instead of having this nice interface (the exact look depends on the OS):
    ![Screenshot of rofi](Images/xournalGradeExamAddStudent.jpg)
- By default, we also change the content of the clipboard based on the currently graded question.
**If you are too lazy to install these dependencies**, you can also simply manually type the name of the student (use TAB or `|` to separate student ID, first name, last name…) and we will do our best to match it with the reference list by column when exporting.

## Usage

Our plugin is relatively simple and does nothing magic: everything is stored in a simple text area so that you can annotate/read/share your exams even without any installed plugin (it would just be longer to write them and navigate in the document, hence this plugin).
...
1. First, make sure to have all exams presented in a single PDF file. For this we provide multiple ways to do that:
    ...
    - If the exams are spread across multiple files (e.g. because they came in digital form, if you scanned them in multiple batches, or if you took pictures) you can assemble all PDF in the current folder by clicking on on entry of the form `Plugins > GradeExam: merge all PDF/images in current folder (images in {portrait/landscape/exif})` (each entry applies different rotations to the images). The path of the resulting file is shown once produced and no warning is shown if a file with the same name already exists.
    - Then if needed you should "unbook" the scanned double page into regular A4-like documents. For this, just click the menu entry named like `Plugins > GradeExam: unbook A3 to A4`, and it will put the newly unbooked PDF `*_unbook.pdf` next to the current PDF (check the message at the end for the exact path). Warning, if the file already exists it will be overwritten without warnings.
...
7. Then, **grades** are added in each exam next to the question they refer to, and the question name is separated from the given points via `~>` (faking an arrow without using too common symbols) so that writing `1.3 ~> 2` means that you gave 2 points to question 1.3. The points can either be **regular points** or a **percentage** like `1.3 ~> 100` to give all points, depending on the export mode, as for now you can't mix points and percentages in the same document. Multiple grades can be added in the same text area if they are separated by newlines. So far, xournal++ lacks an appropriate Lua API to add text on click (I plan to give it a try at some points), so instead we automatically populate the clipboard to contain the grade that should be graded for the current student. If you don't want your clipboard to be modified, you can also disable this via `Plugins > GradeExam/Advanced: toggle put grade in clipboard` and simply manually create this text. Note that if the student has **not written questions in the order** specified in the subject (e.g. exercice 5 comes before exercice 4) you can add an empty grade like `5.1 ~>` next to the beginning of ex 5, skip the rest of it, and continue to grade exercice 4 as before, and when navigating later (and if you added the list of reference grades in the preamble), you will be moved directly to exercice 4 until you have finished its correction, and then when correcting question `5.1` you will be moved back to where the student wrote their exercice 5.
8. After correcting one student, you want to **navigate to the next student/question to correct**. We provide a number a functions to navigate the document, but the most handy is certainly the function associated to the shortcut F4 `Plugins > GradeExam: 1st uncorrected grade all doc & save`:
    - On modern xournal++ versions, this goes to the next student/question to correct in the whole document. More precisely, this finds the student with the smallest uncorrected (= empty) grade and moves to it (if a question with empty points like `5.1 ~>` has been created as explained above if the student has not ordered questions properly) or to the question right before it otherwise (e.g. `4.6`) by searching for a text area that you added containing `4.6 ~> …`. This has a number of useful implications: if a student has not finished an exercice, you can just add grades setting the score `0` to all remaining questions of this exercice and **this student will be skipped** until you finish to correct this exercice. Also, for this to work better you should add a list of reference grades that will be used to know, for instance, that question 5.1 comes right after question 4.6, since e.g. otherwise we can't know if you should be moved to `5.1` (to start to correct exercice 5) or `4.6` (to correct a question `4.7`) after correcting the question `4.6`. 
    - On old xournal++ versions no API is provided to efficiently do that hence we just move to the last uncorrected grade of the next student. For now we also don't change the clipboard on old xournal++ so you need to type the grade manually (or copy/paste it yourself).

    By default grades are sorted based on their order in the reference list of grades, and if not present we sort them naturally by splitting them at each dot and comparing their numeric value if available and lexicagraphically otherwise, so that e.g. `1.5` comes before `10.4`. You can then grade this question as explained in the previous step, press F4 again to move to the next question to correct… and repeat until you have corrected all questions on all exams.
8. You can then **grade** this question. On modern xournal++ versions, we directly populate the clipboard with a template like `5.1 ~>` that you can paste in a new text area and complete by adding the grade (in points like `5.1 ~> 2.5` or in percentages like `5.1 ~> 100`) at the end. On older versions, you need to type it yourself (or copy/paste the template yourself). If you don't like the fact that the clipboard is changed automatically you can disable (or re-enable) it in `Plugin > GradeExam/Advanced: toggle put grade in clipboard`. For now we enable this by default but this may change in futur versions. Also this is not saved in any configuration file for now so you need to apply it when you re-open any new document.
9. (optional) You can also add comments right below the grade after a new line. Separate different comments with an empty line. Previously added comments can be put into the clipboard by pressing F7 or clicking on `Plugin > GradeExam: copy comment to clipboard` so you can directly paste it in the current editing box. Comments will be exported into the YAML file so you can print comments for each student separately.
10. If a student has not finished the exam, you can put zeros to all the remaining non-graded grades (based on the list of reference grades, or all grades in the document if not available). For this, just click on `Plugin > GradeExam: put to clipboard all remaining grades to zero`: this will populate your clipboard with the correct text that you can paste in a new text area.
11. (optional) On the preamble (empty pages before the first exam) you can also add a text area starting with `*settings:` followed by a new line and a list of options (one per line) like `my option=my param` to **customize various aspects of this program**. For now we only provide one option `removeNoGradeCells` such that, if it is set to `removeNoGradeCells=true`, will export in the CSV any cell with no grade as an empty cell instead of writting a more visible `NO GRADE` text.
12. After correcting all students, you can **export the grades in a CSV file** by clicking on an entry named like `Plugins > GradeExam: export CSV and YAML` or `Plugins > GradeExam: export CSV (percent formula) and YAML` if you graded each question in percent. A csv file named like `*_grades.csv` and a YAML file `*_grades_with_comments.yml` will be created with all grades, and a list of warnings may be printed if it detected weird things (e.g. a student with two grades for the same question…). A warning is also shown for each page that has zero annotations to ensure that you have not skipped a page by mistake, so write a dummy text like `seen` on pages with no grade to ensure that it is not a forgotten page. **If the list of warning is too long** the `OK` button may be hidden, forbidding you from clicking it to dismiss the list of warnings. Just press the Enter key instead (all logs are also visible in a `.log` file created next to the document).
